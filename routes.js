...
router.get("/conversations/:id", protect(["user", "provider"]), async (req, res, next) => {
  try {
    ...
    // Sécurité : seul les 2 participants
...
router.post("/bookings", protect(["user"]), async (req, res, next) => {
  try {
    ...
    if (io) io.to(`user:${booking.provider}`).emit("notification", {
      type: "booking", title: "Nouvelle réservation", message: `Le ${new Date(booking.date).toLocaleDateString("fr-FR")}`,
    });
...
/* ══════════ PAIEMENT (mock Stripe / CIB / Edahabia) ══════════ */
router.post("/payments/create", protect(["user"]), async (req, res, next) => {
  try {
    const { bookingId, amount, method } = req.body;

    // ⚠️ Ici tu brancherais Stripe ou SATIM (CIB/Edahabia)
    // Pour la démo, on simule une intention de paiement
    ...
    // En prod : renvoyer le clientSecret Stripe ou l'URL SATIM
    res.status(201).json({
      paymentId: payment._id,
      transactionId: payment.transactionId,
      // URL de redirection pour SATIM ou Stripe Checkout
...
router.get("/quotes/:id/pdf", protect(["provider", "user", "admin"]), async (req, res, next) => {
  try {
    ...
    const { Provider } = await import("./models.js");
